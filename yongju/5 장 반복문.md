...
- SQL에는 명시적인 반복문이 없습니다. 그래서 보통의 사용자들은 호스트 언어(ex : java)에서 반복문을 구현한 후 SQL을 반복적으로 호출하거나, 값 업데이트를 합니다. 반복을 사용해 문제 처리시 아래와 같은 문제가 있습니다. **( 이제 반복을 사용한 SQL을 반복계, 여러행을 한꺼번에 처리하는 SQL을 포장계라고 부릅니다. )**
